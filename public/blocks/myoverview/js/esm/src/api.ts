...
/**
 * Web-service layer for the course overview block (MDL-88965).
 *
...
export const PREF_HIDDEN = (courseId: number): string => `block_myoverview_hidden_course_${courseId}`;
 ...
 * @returns The courses page and the next offset.
 */
...
export async function setCourseHidden(courseId: number, hidden: boolean): Promise<void> {
    return writePreference(PREF_HIDDEN(courseId), hidden ? "1" : null);
}
