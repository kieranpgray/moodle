...
Feature: My overview block searching
  ...
  Scenario: Searching keeps the active filter applied
    ...
    # Searching must stay scoped to the Starred filter: a non-starred match must not appear.
    When I set the field "Search courses" in the "Course overview" "block" to "Course"
    Then I should see "Course 01" in the "Course overview" "block"
    And I should see "Course 02" in the "Course overview" "block"
    And I should not see "Course 03" in the "Course overview" "block"
    And I log out
